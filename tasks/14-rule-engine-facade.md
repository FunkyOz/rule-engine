---
title: Rule Engine Facade
status: done
priority: High
description: Implement the main RuleEngine class as the primary entry point for rule evaluation
---

## Objectives
- Create the main `RuleEngine` class as the facade
- Provide simple API for rule evaluation
- Support multiple evaluation strategies
- Enable easy configuration and extension

## Deliverables
1. `src/RuleEngine.php`

## Technical Details

### RuleEngine

The main facade class for the rule engine.
...
// Build and add rules using the fluent API
$engine->addRule(
    $engine->builder()
        ->name('adult_user')
...
$engine->addRule(
    $engine->builder()
        ->name('premium_member')
        ->when('user.subscription')->in(['premium', 'enterprise'])
        ->then()
        ->meta('discount', 0.2)
        ->build()
);

// Evaluate with array context
$context = [
    'user' => [
        'age' => 25,
...
$isAdult = $engine->evaluate('adult_user', $context); // true
$isPremium = $engine->evaluate('premium_member', $context); // true

// Evaluate all rules
$allPass = $engine->evaluateAll($context); // true

// Get detailed results
$results = $engine->evaluateAllWithResults($context);
foreach ($results as $result) {
    echo $result->getRuleName() . ': ' . ($result->passed() ? 'PASS' : 'FAIL') . "\n";
}

// Get passing rules to apply their effects
...
- Task 13 - Rule Builder

## Estimated Complexity
**Medium** - Facade pattern coordinating all components

## Implementation Notes
- RuleEngine is the main entry point for users
- Accepts both array and ContextInterface for flexibility
- Pre-registers all default operators
- Custom operators can be registered after creation
- Builder method provides fluent API access
- Consider adding event hooks for rule evaluation in the future

## Acceptance Criteria
- [x] `RuleEngine::create()` creates engine with default operators
- [x] `builder()` returns a configured RuleBuilder
- [x] `addRule()` and `addRules()` manage rules
- [x] `evaluate()` evaluates single rule by name
- [x] `evaluateAll()` and `evaluateAny()` work correctly
- [x] `evaluateAllWithResults()` returns detailed results
- [x] Array context is automatically converted to Context
- [x] Custom operators can be registered
- [x] `RuleNotFoundException` thrown for missing rules
